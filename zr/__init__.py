...
def excel_report_file_to_df(excel_file, sheetname=0):
    ...
    if df.iloc[0, 1] == 'Unnamed: 1':
        ...
        df = df.iloc[
            3:-1
        ].copy()  # get the data (from row 3, till the penultimate row (last row contains stats))
...
def get_client(clientCustomerId='7998744469'):
    ...
    headers = {
        'email': os.getenv('VEN_ADWORDS_EMAIL'),
...
def download_report(
    ...
    if download_format == 'df':
        google_report = report_downloader.DownloadReportWithAwql(
            report_query_str, 'TSV'
        )
        return report_to_df(google_report, thousands, dtype)
    else:
        return report_downloader.DownloadReportWithAwql(
            report_query_str, download_format
        )
...
def mk_report_query_str(
    ...
    if isinstance(varList, str):
        if (
            varList.find(',') == -1
        ):  # if you do not find a comma get the string listing the vars, using varList as a group name
            varList = get_var_list_str(group=varList, source=source)
    else:
        if not isinstance(varList, list):
            raise ValueError(
                'varList must be a comma seperated string or a list of strings'
            )
    ...
    if isinstance(varList, list):
        varList = ', '.join(
            varList
        )  # make a comma separated string from the varList list
...
def get_df_concatination_of_several_accounts(
    account_list, varList=None, number_of_days=300
):
    if varList is None:
        varList = 'Query, Impressions, AdGroupName, CampaignName, AdGroupStatus'
    report_query_str = mk_report_query_str(
        varList=varList,
        source='SEARCH_QUERY_PERFORMANCE_REPORT',
...
def get_var_list_str(group='default', source='SEARCH_QUERY_PERFORMANCE_REPORT'):
    if source == 'SEARCH_QUERY_PERFORMANCE_REPORT':
        var_list = {
            'default': (
                'Query, AdGroupId, AdGroupName, AveragePosition, '
                'CampaignId, CampaignName, Clicks, Cost, Impressions, KeywordId, KeywordTextMatchingQuery, '
                'MatchType, MatchTypeWithVariant'
            ),
            'q_kmv_picc': (
                'Query, AveragePosition, KeywordTextMatchingQuery, MatchType, MatchTypeWithVariant, '
            ...
            'q_matts_pick': (
                'Query, Impressions, AdGroupName, CampaignName, '
        ...
        var_list = {
            'default': (
                'KeywordText, KeywordMatchType, Max. CPC, AdGroupName, CampaignName, '
            ...
            'q_kmv_picc': (
                'KeywordText, KeywordMatchType, Max. CPC, '
...
def import_account_str_to_id(
    source='/D/Dropbox/dev/py/data/aw/account_name_accountid.csv',
...
lu_name_x_dict = {
    ...
    'budget_explicitly_shared': ['budgetExplicitlyShared', 'Budget explicitly shared'],
    ...
    'content_lost_is_budget': ['contentLostISBudget', 'Content Lost IS (budget)'],
    ...
    'conv_rate_many_per_click': ['convRateManyPerClick', 'Conv. rate (many-per-click)'],
    'conv_rate_many_per_click_ace_indicator': [
        'convRateManyPerClickACEIndicator',
        'Conv. rate (many-per-click) ACE indicator',
    ],
    'conversion_action_name': ['conversionActionName', 'Conversion action name'],
    ...
    'cost_conv1_per_click': ['costConv1PerClick', 'Cost / conv. (1-per-click)'],
    ...
    'display_network_max_cpc': ['displayNetworkMaxCPC', 'Display Network max. CPC'],
    ...
    'keyword_placement_state': ['keywordPlacementState', 'Keyword/Placement state'],
    ...
    'most_specific_location': ['mostSpecificLocation', 'Most specific location'],
    ...
    'position_ace_indicator': ['positionACEIndicator', 'Position ACE indicator'],
    'ptr': ['ptr', 'PTR'],
    ...
    'this_extension_vs_other': ['thisExtensionVsOther', 'This extension vs. Other'],
    ...
    'value_conv1_per_click': ['valueConv1PerClick', 'Value / conv. (1-per-click)'],
...
xml_x_dict = {
    ...
    'budget': ['Budget', 'budget'],
    'budgetExplicitlyShared': ['Budget explicitly shared', 'budget_explicitly_shared'],
    ...
    'contentLostISBudget': ['Content Lost IS (budget)', 'content_lost_is_budget'],
    ...
    'convRateManyPerClick': ['Conv. rate (many-per-click)', 'conv_rate_many_per_click'],
    'convRateManyPerClickACEIndicator': [
        'Conv. rate (many-per-click) ACE indicator',
        'conv_rate_many_per_click_ace_indicator',
    ],
    'conversionActionName': ['Conversion action name', 'conversion_action_name'],
    'conversionOptimizerBidType': [
        'Conversion optimizer bid type',
        'conversion_optimizer_bid_type',
    ],
    ...
    'costConv1PerClick': ['Cost / conv. (1-per-click)', 'cost_conv1_per_click'],
    'costConv1PerClickACEIndicator': [
        'Cost/conv. (1-per-click) ACE indicator',
        'cost_conv1_per_click_ace_indicator',
    ],
    ...
    'displayNetworkMaxCPC': ['Display Network max. CPC', 'display_network_max_cpc'],
    ...
    'keywordPlacementState': ['Keyword/Placement state', 'keyword_placement_state'],
    ...
    'positionACEIndicator': ['Position ACE indicator', 'position_ace_indicator'],
    'ptr': ['PTR', 'ptr'],
    ...
    'thisExtensionVsOther': ['This extension vs. Other', 'this_extension_vs_other'],
    ...
    'url': ['URL', 'url'],
    'valueConv1PerClick': ['Value / conv. (1-per-click)', 'value_conv1_per_click'],
    'valueConvManyPerClick': [
        'Value / conv. (many-per-click)',
        'value_conv_many_per_click',
    ],
...
disp_x_dict = {
    ...
    'Budget explicitly shared': ['budget_explicitly_shared', 'budgetExplicitlyShared'],
    ...
    'Content Lost IS (budget)': ['content_lost_is_budget', 'contentLostISBudget'],
    ...
    'Conversion action name': ['conversion_action_name', 'conversionActionName'],
    'Conversion optimizer bid type': [
        'conversion_optimizer_bid_type',
        'conversionOptimizerBidType',
    ],
    ...
    'Cost': ['cost', 'cost'],
    'Cost / conv. (1-per-click)': ['cost_conv1_per_click', 'costConv1PerClick'],
    'Cost / conv. (many-per-click)': [
        'cost_conv_many_per_click',
        'costConvManyPerClick',
    ],
    ...
    'Display Network max. CPC': ['display_network_max_cpc', 'displayNetworkMaxCPC'],
    ...
    'Keyword/Placement state': ['keyword_placement_state', 'keywordPlacementState'],
    ...
    'Most specific location': ['most_specific_location', 'mostSpecificLocation'],
    ...
    'Position ACE indicator': ['position_ace_indicator', 'positionACEIndicator'],
    ...
    'This extension vs. Other': ['this_extension_vs_other', 'thisExtensionVsOther'],
    ...
    'Value / conv. (1-per-click)': ['value_conv1_per_click', 'valueConv1PerClick'],
    'Value / conv. (many-per-click)': [
        'value_conv_many_per_click',
        'valueConvManyPerClick',
    ],
